...
class BookingItem(models.Model):
    ...
    cxn_internal_code = fields.Char(related='rezdy_product_id.product_tmpl_id.default_code')
    total_quantity = fields.Integer(string='Total Quantity')
    amount = fields.Monetary()
    subtotal = fields.Monetary()
    total_item_tax = fields.Monetary()
    order_line_ids = fields.One2many(
        comodel_name='sale.order.line', inverse_name='rbi_id', string="Order Lines", readonly=True
    )
    ...
    flight_time = fields.Char(string='Flight Time')
    flight_datetime = fields.Char(string='Flight Date and Time')

    bkg_item_ref = fields.Char(string="Booking Item Reference")
    bkg_item_guid = fields.Char(string="Booking Item GUID")

    def _compute_pickup_ref(self):
        for rec in self:
            rec.pickup_ref = extract_ref(rec.pickup)

    def _compute_drop_off_ref(self):
        for rec in self:
            rec.drop_off_ref = extract_ref(rec.drop_off)
